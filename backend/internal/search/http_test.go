...
func TestHTTPHandlerServesOwnerAndPublicPaginatedSearch(t *testing.T) {
	repository := &repositoryStub{page: Page{
		Results:    []Result{{ID: "11111111-1111-4111-8111-111111111111", SpaceID: "22222222-2222-4222-8222-222222222222", Title: "Memory", Slug: "memory", Snippet: "memory body", Rank: 1.5}},
		Page:       1,
		PageSize:   20,
		TotalItems: 1,
	}}
	...
	for _, path := range []string{"/api/v1/search?q=memory&page=1&pageSize=20", "/api/v1/public/search?q=memory&page=1&pageSize=20"} {
		...
		if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), "memory body") {
