  ...
  return (
    <main>
      ...
      <section className="space-page-header">
        <Link className="back-link" href="/"><ArrowLeft size={16} /> All spaces</Link>
        <p className="section-kicker">Public Knowledge Space</p>
        <h1>{space.name}</h1>
        <p>{notesResponse.pagination.totalItems} published {notesResponse.pagination.totalItems === 1 ? "note" : "notes"}</p>
      </section>
      <section className="note-index" aria-label={`${space.name} notes`}>
        {notesResponse.data.length === 0 ? (
          <div className="empty-state"><h2>No notes published here</h2><p>This space is public, but its first note is still being prepared.</p></div>
        ) : (
          notesResponse.data.map((note, index) => (
            <article className="note-index-row" key={note.id}>
              ...
              <Link className="round-link" aria-label={`Read ${note.title}`} href={`/notes/${note.id}/${note.slug}`}><ArrowRight size={18} /></Link>
            </article>
          ))
        )}
      </section>
